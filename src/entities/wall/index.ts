...
export class Walls {
  ...
  draw2D(
    ...
    const margineds = createShapeFromPoints(this.walls);
    ...
    this.walls.forEach((wall) => {
      ...
      wall.points.forEach((point: Point, pointIndex: number) => {
        const screenX = point.x * zoomLevel + panOffset.x
        const screenY = point.y * zoomLevel + panOffset.y
        const isDragged = draggedWallIndex !== null && draggedWallIndex === this.walls.indexOf(wall) && pointIndex === draggedPointIndex
        drawPoint(ctx, screenX, screenY, isDragged ? '#1890ff' : '#333')
        if (isDragged) {
          ctx.strokeStyle = '#1890ff'
          ctx.lineWidth = 2
