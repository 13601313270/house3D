...
export class PointGroupEntity extends PointEntityClass<PointGroupData> {
  name: string = '组'
  type: string = 'pointGroup'
  isPointObj: boolean = true
  private circleRadius = 6

  draw2DPreviewByData(ctx: CanvasRenderingContext2D, data: PointGroupData, panOffset: Point, zoomLevel: number): void {
  ...
  private getSize(): { width: number, height: number, depth: number } {
    return {
      width: 100,
      height: 120,
      depth: 80
  ...
  matchHandelInfo(x: number, y: number) {
    ...
    const { width, height } = this.getSize();
    const drawAngelLength = Math.min(width, height);// Math.max(width / 2, this.circleRadius * 2) * 0.9;// 0.9避免超过方块范围
    // 控制点向着angleY角度延伸10个单位后的坐标
    const rotatedXAdd = data.x + Math.cos(data.angleY) * drawAngelLength
    const rotatedYAdd = data.y - Math.sin(data.angleY) * drawAngelLength
